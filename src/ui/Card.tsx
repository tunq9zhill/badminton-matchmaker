import type { ReactNode } from "react";

export function Card(props: { children: ReactNode; className?: string }) {
  return (
    <div className={`soft-fade-up squircle rounded-3xl bg-white shadow-sm border border-slate-200 overflow-hidden flex flex-col transition-all duration-[600ms] ease-[cubic-bezier(0.265,1.296,0,0.985)] ${props.className ?? ""}`}>
      {props.children}
    </div>
  );
}

export function CardHeader(props: { title: string; right?: ReactNode }) {
  return (
    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100">
      <div className="text-sm font-semibold">{props.title}</div>
      {props.right}
    </div>
    
  );
}

export function CardBody(props: { children: ReactNode; className?: string }) {
  return <div className={`px-4 py-3 overflow-hidden ${props.className ?? ""}`}>{props.children}</div>;
}
